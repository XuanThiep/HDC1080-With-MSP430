...
/* @Brief	: 	Initialize module UART, clock source supply is SMCLK, FSMCLK = 8E6 Hz.
 * 				The baud rate is 115200, 8 bit data frame, 1 bit stop, no parity check.
 * 				Receive Interrupt is enabled.
 *
 * @Para	:	+ USCI_base_address - This is base address of USCI Module.
 *
 * 				+ TxD_PORT - This is GPIO PORT of TxD pin
 * 				+ TxD_PIN  - This is GPIO PIN of TxD pin
 *
 * 				+ RxD_PORT - This is GPIO PORT of RxD pin
 * 				+ RxD_PIN  - This is GPIO PIN of RxD pin
 * @Return	:	None
 * @Note	:   None
 */
void UART_Initialize(uint16_t USCI_base_address,
		uint8_t TxD_PORT,uint16_t TxD_PIN,
		uint8_t RxD_PORT,uint16_t RxD_PIN)
{
	USCI_A_UART_initParam para;
	GPIO_setAsPeripheralModuleFunctionInputPin(TxD_PORT,TxD_PIN);
	GPIO_setAsPeripheralModuleFunctionInputPin(RxD_PORT,RxD_PIN);

	/* UART clock frequency = SMCLK = 8E6
	 * Baudrate = 115200 => prescaler = UCBRx = 4
	 * UCBRFx = 3, UCBRSx = 5, UCOS16 =1.
	 *
	 * From: Table 36-5. Commonly Used Baud Rates, Settings, and Errors, UCOS16 = 1 at Page 953/1189 User guide.
	 */
	para.selectClockSource = USCI_A_UART_CLOCKSOURCE_SMCLK; // 8E6 Hz
	...
	USCI_A_UART_init(USCI_base_address,&para);
	USCI_A_UART_enable(USCI_base_address);
	USCI_A_UART_clearInterrupt(USCI_base_address,USCI_A_UART_RECEIVE_INTERRUPT_FLAG);
	USCI_A_UART_enableInterrupt(USCI_base_address,USCI_A_UART_RECEIVE_INTERRUPT);
